...
import type { Dossier, DossierMeta } from "@/core/domain/dossier";

const CONTENT_DIR = join(process.cwd(), "content");

export type ModuleMeta = DossierMeta;
export type Module = Dossier;
...
function extractTitle(raw: string): string {
  ...
  return m ? m[1].trim().replace(/^Module\s+\d+\s+—\s+/i, "") : "(sans titre)";
}
